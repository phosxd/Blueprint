...
static func add_blueprint(name:String, blueprint:Blueprint) -> void: ## Registers the Blueprint. Does nothing if the given name is already in use. Automatically called when a Blueprint is created.
...
static func add_blueprint_from_file(name:String, filepath:String) -> void: ## Registers a Blueprint from a JSON file. Does nothing if error occurs.
